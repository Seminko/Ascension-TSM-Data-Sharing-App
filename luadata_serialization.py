"""
Original library: https://github.com/leafvmaple/luadata
Updated a bit so that it adheres to WoW formatting
"""
...
def serialize(var, encoding="utf-8", indent=None, indent_level=0):
    ...
    if var is None:
        ...
        for kv in entries:
            ...
            # insert indent
            if indent is not None:
                parts.append(indent * (indent_level + 1))
            # insert key
            if nohash:  # pure list: do not need a key
                ...
                parts.append("[")
                parts.append(serialize(key, encoding, indent, indent_level + 1))
                parts.append("]")
                parts.append(s_tab_equ)
            # insert value
            parts.append(serialize(val, encoding, indent, indent_level + 1))
            parts.append(",")
            if indent is not None:
                parts.append("\n")
            lastkey = key
